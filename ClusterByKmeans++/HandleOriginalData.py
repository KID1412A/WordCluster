...
def chinese_word():
    ...
    chinese_character = GetWord.real_word(chinese_character)
    # chinese_character = ' '.join(chinese_character.split())  # 连续多个空格合并为一个空格
    write_file(new_cut_word_filePath, chinese_character)
    # end_time = time.time() #结束计时
    # print(('clust word time cost', end_time-start_time, 's')) #计算聚类耗时
...
def useful_word():
    ...
    useful_chinese_word = UsefulWord.format_str(useful_chinese_word)
